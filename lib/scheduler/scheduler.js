...
(function() {
  ...
  this.spawnWorker_p = function(appSpec){
    var self = this;
    var key = appSpec.getKey();

    var defer = Q.defer();
    this.$workers.push(defer.promise);

    var workerId = this.$workers.length-1;
    ...
    .then(function(listenPort) {
      ...
      exitPromise
      .fin(function() {
        self.releaseWorker(self.$workers[workerId]);
        self.$workers.splice(workerId, 1);
